...
> 〔Owner 決策單 2026-09-26 第四輪 T1～T3〕Owner 在「上線與本機速度決策單」確認了第 5.6.6 節的待裁決（T1）與實作時自己定的規則（T2、T3），**三題都選 1、照現狀**，程式不改；就地加註在第 5.6.5 節彙總表之前與第 5.6.6 節（原文不刪）。總表見 `docs/HANDOFF.md` §0.00b。
...
> 〔Owner 決策單 2026-09-26 第四輪 T2、T3〕Owner 在決策單上確認了其中列出的規則，兩題都選 1、照現狀：
> - T2（決策單列的 ①～⑧，全部照現狀）：①「要重練」勾選框每一題新題都能勾（含答對的題），預設不勾；②承上組：勾一題整組進清單，「移出」整組移出、「重新加入」整組回來，「判定已會」只動那一題；③練到會或移出後「重新加入」從第 1 關重來，錯的次數不歸零；④進行中的題按「重新加入」只提示、不歸零進度；⑤派出去超過 14 天沒批改在清單上提醒；⑥關卡名稱：第 1 關「錯題重練」、第 2 關「一週回測」、第 3 關「兩週回測」；⑦新題附上重練題的卷，卷名用第一個新題的章節（第 5.6.6 節第 4 條）；⑧重練題放不下時，錯誤訊息建議可以改成的題數（第 5.6.3 節 ①）。
> - T3：Word 詳解版只在答案與詳解段標「（重練）」，題目段不標（下表「Word」列、第 5.6.3 節 ⑦）。
...
| (3) | 「判定已會」的題被承上組帶著出又答錯時維持練到會（第 4.4 節兩列規則衝突） | ⏸ **待 Owner 裁決**，純函式不改（見下）。〔Owner 決策單 2026-09-26 第四輪 T1〕✅ **已決定**：維持練到會、只提示（見下）。批改摘要不會誤導：這一題不算 `reset`（狀態沒變）；批改卡儲存後另外提示「有 N 題重練題已「判定已會」，答錯不改狀態（仍是練到會；要再練請到錯題重練卡按「重新加入」）。」（與第 5 條同一次 API-1 對照，不改 API-10 的回應形狀）。測試：`retrainUi.test.js`、`retrainFlow.pg.test.js` |
...
〔Owner 決策單 2026-09-26 第四輪 T1〕**已決定**：Owner 選 1「維持目前做法（尊重老師的判定，只提示）」——「判定已會」（`teacher_override = 'mastered'`）的題被同組帶著出又答錯，仍是練到會、錯的次數＋1，批改卡照上面第 (3) 列提示「要再練請按重新加入」。凍結的純函式與測試都不改；第 4.4 節兩列規則的衝突以「`teacher_override` 重算時優先」為準。
...
**與設計稿（凍結版）不同、或設計稿沒寫而由本次實作決定之處**（列給 Owner 確認）〔Owner 決策單 2026-09-26 第四輪 T2〕決策單 T2 的 ⑦（混合卷卷名）就是本表 ④，Owner 選 1 照現狀；本表其餘各條（①②③⑤⑥）照現狀運作，決策單沒有逐條列出。
