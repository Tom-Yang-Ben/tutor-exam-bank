...
- 修正後只剩題 47：它是 2026-05 從舊系統匯入的殘缺版本，同一題已於 2026-08-29 以乾淨 LaTeX 重新入庫（題 195）；題 47 已於同日隨舊系統重複版本一併刪除。
- 題庫清理後（198 題，刪除 10 題重複舊版與 11 題不在原卷庫的題）重跑：lint 的 error 級問題為 **0**。
- warn 級 `bare_script_text`、`slash_fraction`（健檢時 118、79 處，清理後 115、65 處）幾乎全在舊題的純文字（`1/4`、`m/s^2`），網頁與 Word 都照字面印，不跑版。
