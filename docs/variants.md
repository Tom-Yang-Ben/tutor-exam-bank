...
| `VARIANT_OFFTOPIC_SIM_MIN` | `0.92` | `agents/generateVariant.js`（經 `ctx.config.thresholds.variantOfftopicSimMin`） | 「同概念換數字」100% 過、跨章誤收 0.64%（3.2 節） |
...
2. ~~`gate_pass_rate` 尚未有數字~~ → **2026-08-24 已錄，0.15**（第 4 節）。真正的缺口變成：`VARIANT_OFFTOPIC_SIM_MIN=0.92` 擋掉 26/30 藍本；是否下修（0.88～0.90）屬裁決，改了要重錄。
