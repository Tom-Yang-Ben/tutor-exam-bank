...
5. **比數字與英文字母**〔審查修正 2026-09-26〕：中文字 5-gram 幾乎不看數字——題庫的題與原卷「中文一樣、數字不同」（不同學校、年度的同一題）時覆蓋率照樣是 1，圖上的數值卻可能不同。所以**每一題**都拿題幹的數字 0–9 與小寫英文字母 a–z（LaTeX 裡的數字照算，例：`$10$`、`\frac{1}{2}`；指令名與「[附圖描述：…]」不算；兩邊都去掉行首題號與配分註記，與原卷比對同一套 `tally`）比原卷**該題的段落**（`questionSegment`）：起點那一行到題幹最後一個對得上的字所在的行，再往下接同頁同欄、往下走、沒有中文字的行（只有數字的選項，例：「(A) 5 (B) 6」），遇到題號行、大題標題、有中文字的行或頁尾（頁面最下方 5%）就停；**圖框裡的字（頂點標註、座標刻度）不算**。相符度＝兩邊多重集合的 Jaccard（交集 ÷ 聯集，重複照次數算）；有任何一個字對不上就無條件捨去到小數兩位且最多 0.99（不會顯示成 1.00）；兩邊都沒有數字與字母時不比。**大寫字母不比**：選項代號 (A)–(E)、幾何頂點標註常只有一邊有，比了會一直誤報（代價：只差頂點名稱 ABC／PQR 的兩個版本分不出來）。
