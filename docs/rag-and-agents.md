...
- 金字塔：`npm test` 1,476 項（2026-09-15）不連網不連庫；整合測試對 tmpfs 測試庫（`--test-concurrency=1`）；
...
4. **學生姓名不出境**（2026-09-15 補）：主控 LLM 看到的學生一律是「學生#<id>」——
   老師的訊息、對話歷史、工具參數與工具結果進 prompt 前經 `utils/pseudonym.js` 遮罩，
   主控回傳的參數與最終回覆再換回姓名；NLQ 的 LLM 輔路徑與 embedding 同一套。
   沒有學生清單時遮罩是恆等函式，所以 cassette 鍵與 eval 數字一個都沒變。
   起因是面試準備查核發現 DEC-009 寫「資料留本地」，但這兩條路徑把姓名送進了 Gemini。
