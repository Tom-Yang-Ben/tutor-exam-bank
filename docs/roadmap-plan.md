...
>
> 〔修訂 2026-09-24〕階段 5「教學診斷平台」不在本檔展開規劃，只在 [§7](#sec-stage5) 留一句話與連結；其契約是 [`interfaces-stage5.md`](interfaces-stage5.md)。
...
| 階段 5 教學診斷平台〔修訂 2026-09-24〕 | [§7 階段 5](#sec-stage5)（一句話＋連結；契約與裁決 S5-* 在 `interfaces-stage5.md`） |
...
**不交付：** 聊天介面（Non-goal）；學生端登入與自助作答；圖表函式庫；OCR 自動批改；題庫之外的知識檢索。〔修訂 2026-09-24〕其中「題庫之外的知識檢索」與「OCR 自動批改」兩項已由 **DEC-014** 撤銷（產品定位改為教學診斷平台；`engineering_docs/01_requirements/requirements_tracker.md`）：前者由階段 5 的 AI 家教落地（FR-034），後者撤銷限制但尚未實作；「學生端登入與自助作答」維持延後，其餘兩項不變。本段原文保留為階段 3 當時的凍結規劃。變式題**不開專用通道**——入庫後是一般題，`generatePaper` 照章節抽得到；唯一額外規則是「同一 `variant_of` 家族在同一張卷只取一題」。
...
10. ~~舊系統匯入題的殘缺重複版本~~ → **已執行（2026-09-15，owner 裁定）**。2026-05 由 MySQL 匯入的
   ...
   另 12 題（28–39）以 `Desktop/各校考卷` 八份原卷逐題比對：題 38 為台中一中 112-1 第 3 題（改標），
   其餘 11 題不在任何一份原卷，owner 裁定刪除。刪除前皆查證無 attempts、變式、試卷與 job 引用，
   原資料備份於本機暫存。清理後題庫 198 題，來源標記全數對應八份原卷或自製，無 `unknown`。
11. **「承上題」與去重的斷鏈** → **三段皆已合併進 main（2026-09-16）**〔修訂 2026-09-16b〕。
   原問題（2026-09-15 全庫解答健檢發現）：題庫沒有「題組」概念，承上題與其上一題各自獨立入庫，
   上一題若被 dedup 判重複而不入庫，承上題就失去脈絡（題 169、198 即是）。
   最終做法（DEC-012、FR-019）：不改 extract prompt／schema（cassette 不失效），改在伺服器端以
   regex 偵測「承上(一)題」，以自我外鍵 `questions.follows_question_id` ＋ `follows_src`
   （pipeline／review／backfill／human，human 不被重算覆寫）鏈結前題；上一題被判重複時綁到庫內既有題。
   - ① **資料＋管線＋複核＋回填**：PR #28 已合併（main `f29ddb2`），`migrations/0008_follow_up.sql`。
     2026-09-15 正式庫已套用並回填 7 題（18→17、21→20、26→25、169→13、170→168、198→60、199→63），
     再跑為 0（冪等），無待人工綁定。
   - ② **組卷整組抽取、相鄰排列**：PR #33 已合併（main `8072795`）〔修訂 2026-09-16b〕；放不下整組時的預設見第 14 項。
   - ③ **整組刪除／封存、承上題不可當變式藍本、前端「承上 #id」徽章**：PR #32 已合併（main `27f56ab`）〔修訂 2026-09-16b〕，含第 15 項。
   - 殘餘限制：沒有任何拆題紀錄可推前題的舊題不會自動綁（回填報告列為待人工，目前 0 題）；
     人工綁定介面尚無，需要時以 `follows_src='human'` 直接寫入。
12. ~~拆題結果對照原卷文字層~~ → **已執行（2026-09-15，feat/source-check；ADR-009、FR-020、DEC-013 待簽核）**〔修訂 2026-09-15f〕。
   ...
   - 合併與上線〔修訂 2026-09-15g〕：與承上題分支合併後 code review 修正行首小數誤判題號等兩處邊界規則
     （重跑校準數字不變），PR #29 已合併（main `7b37be4`）；2026-09-15 正式庫已套用 0009 並重啟服務。
     剩餘問題見第 16、17、23 項。
...
16. ~~預算用盡時零成本節點的失敗原因被改寫~~ → **已修（2026-09-16，PR #31 併入 main `ef787d4`）**〔修訂 2026-09-16b〕。
   原問題：`pipeline/stateMachine.js` 規則 3 在 `budgetLeft <= 0` 時，把 `source_check`（及 dedup）回的
   `fail` 一律改成 `budget_exceeded`，題目因此出現在 retry 清單，重跑後才得到正確的
   `transcription_mismatch`；最終結果正確、不遺失資料。
   修法：零成本節點的 `fail` 不再被改寫成 `budget_exceeded`，首輪即得到正確原因。
17. **已知限制：原卷比對跨 20 頁切塊邊界的題目**〔修訂 2026-09-16b〕。片段可能在塊尾被截斷，理論上可能誤報負號，
...
21. **已知限制（操作）：本機以 `npm run dev`（nodemon）執行時的升級順序**。2026-09-15 `git pull` 帶入
   ...
   部署文件與 README 的步驟說明已隨 PR #31 合併（main `ef787d4`）〔修訂 2026-09-16b〕。
22. **暫緩：階段 4 B 批**（使用者指示擱置，2026-08-24）：私有 golden set、分類閾值 0.88、fixture 擴充至 120 題、
   A-T16 前後對照。與第 1 項 P-16 同屬「本階段完成後再議」。
23. **已知限制：原卷比對不可比對的題目比例**。約 46% 題目跳過（掃描檔整份無文字層 20.5%、
   中文字太少 14.5%、找不到段落 9.4%），選項整排前移抓不到。改善方向是對掃描檔加 OCR，
   但會引入新依賴與誤差來源；待真實誤抄率證明值得再議。
24. ~~平行開發的 4 個 PR（#30–#33）合併時機與順序~~ → **已執行（2026-09-16）**〔修訂 2026-09-16b〕。
