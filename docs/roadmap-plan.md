...
11. **「承上題」與去重的斷鏈** → **部分完成：第一段已上線，第二、三段進行中**〔修訂 2026-09-15g〕。
   ...
   - ① **資料＋管線＋複核＋回填**：PR #28 已合併（main `f29ddb2`），`migrations/0008_follow_up.sql`。
     ...
     再跑為 0（冪等），無待人工綁定。
   - ② **組卷整組抽取、相鄰排列**：進行中（PR #33，分支 `feat/follow-up-paper-group`）；放不下整組時的預設見第 14 項。
   - ③ **整組刪除／封存、承上題不可當變式藍本、前端「承上 #id」徽章**：進行中（PR #32，分支 `feat/follow-up-protect-badge`，含第 15 項）。
...
14. **待決策：組卷時整組承上題放不下的處理**（第 11 項②）。
   ...
   做法：PR #33 已把選項做成單一切換點，owner 拍板後改一行。
15. **進行中：刪除被當作變式題藍本的題回 500**。`jobs.source_question_id` 外鍵衝突未被捕捉，
   應比照 `job_questions` 衝突回 409「請改用封存」。併入 PR #32（分支 `feat/follow-up-protect-badge`）。
16. **進行中：預算用盡時零成本節點的失敗原因被改寫**。`pipeline/stateMachine.js` 規則 3 在
   ...
   PR #31（分支 `fix/pipeline-free-node-budget-reason`）。
17. **進行中（文件）：原卷比對跨 20 頁切塊邊界的題目**。片段可能在塊尾被截斷，理論上可能誤報負號，
   未以真實原卷實測；PR #31 以單元測試釘住現況並寫入 `docs/source-check.md` 限制段。
18. **進行中：Word 匯出不含本機裁切的附圖**（附圖裁切 PR #3 的已知未做）。老師下載的考卷缺圖。
   PR #30（分支 `feat/word-export-figures`）：題幹後插入圖片、依頁寬縮放、缺圖不讓整份匯出失敗、路徑限制在附圖目錄內。
...
24. **待決策：今晚平行開發的 4 個 PR（#30–#33）合併時機與順序**（第 11 項②③、第 15–18、21 項）。
