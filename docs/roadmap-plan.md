...
11. **「承上題」與去重的斷鏈** → **部分完成：第一段已上線，第二、三段進行中**〔修訂 2026-09-15g〕。
   ...
   - ① **資料＋管線＋複核＋回填**：PR #28 已合併（main `f29ddb2`），`migrations/0008_follow_up.sql`。
     ...
     再跑為 0（冪等），無待人工綁定。
   - ② **組卷整組抽取、相鄰排列**：進行中（分支 `feat/follow-up-paper-group`）；放不下整組時的預設見第 14 項。
   - ③ **整組刪除／封存、承上題不可當變式藍本、前端「承上 #id」徽章**：進行中（分支 `feat/follow-up-protect-badge`，含第 15 項）。
...
14. **待決策：組卷時整組承上題放不下的處理**（第 11 項②）。
   選項 A（實作預設）：少出題並在回應附註實際題數與原因；選項 B：回 400 請老師調整題數。
   選 A 的理由：老師要的是一份能用的考卷，少一兩題比整份出不來好；選 B 的理由：題數是老師明確指定的數字。
   做法：分支 `feat/follow-up-paper-group` 已把選項做成單一切換點，owner 拍板後改一行。
15. **進行中：刪除被當作變式題藍本的題回 500**。`jobs.source_question_id` 外鍵衝突未被捕捉，
   應比照 `job_questions` 衝突回 409「請改用封存」。併入分支 `feat/follow-up-protect-badge`。
16. **進行中：預算用盡時零成本節點的失敗原因被改寫**。`pipeline/stateMachine.js` 規則 3 在
   `budgetLeft <= 0` 時把 `source_check`（及 dedup）回的 `fail` 一律改成 `budget_exceeded`，
   題目因此出現在 retry 清單，重跑後才得到正確的 `transcription_mismatch`；最終結果正確、不遺失資料。
   分支 `fix/pipeline-free-node-budget-reason`。
17. **進行中（文件）：原卷比對跨 20 頁切塊邊界的題目**。片段可能在塊尾被截斷，理論上可能誤報負號，
   未以真實原卷實測；同分支 `fix/pipeline-free-node-budget-reason` 以單元測試釘住現況並寫入 `docs/source-check.md` 限制段。
18. **進行中：Word 匯出不含本機裁切的附圖**（附圖裁切 PR #3 的已知未做）。老師下載的考卷缺圖。
   分支 `feat/word-export-figures`：題幹後插入圖片、依頁寬縮放、缺圖不讓整份匯出失敗、路徑限制在附圖目錄內。
...
21. **已知限制（操作）：本機以 `npm run dev`（nodemon）執行時的升級順序**。2026-09-15 `git pull` 帶入
   ...
   部署文件與 README 的步驟說明併入分支 `fix/pipeline-free-node-budget-reason`。
...
24. **待決策：今晚平行開發的 4 個 PR 合併時機與順序**（第 11 項②③、第 15–18、21 項）。
   皆從 main `7b37be4` 分出、各自通過 code review 與 CI；共用文件（測試總數、FR-019 驗收列）可能在後合的 PR 出現衝突，
   合併時由主線統一解。預計皆不含新 migration，合併後只需重啟服務。
