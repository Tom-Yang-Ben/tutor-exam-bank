...
| S5-13 | 助教與 NLQ 的化學支援 | 助教只改工具驗證的科目清單（讀 `SUBJECTS`），工具說明書（SYSTEM 的一部分）仍寫「數學\|物理」；NLQ 的 LLM 輔路徑本階段不支援化學。要支援需另開裁決並重錄兩者的 cassette。〔修訂 2026-09-26 決策單 B5〕Owner 2026-09-25 決策單：**改為支援化學**（助教工具說明書與 NLQ 的 LLM 輔路徑都要能處理化學，兩者的 cassette 需重錄）；由 `dec/b5-chem-assistant-nlq` 實作，合併前程式仍照本列原決定運作。〔修訂 2026-09-26 合併回填〕已合入 `local/integration`（`7dc14a0`）：NLQ 模板 `nlq.v1` → `nlq.v2`（白名單列三科、schema 改讀 `subject_all`／`chapter_all`）、助教 `assistant.v1` → `assistant.v2`（說明書科目由 `SUBJECTS` 產生、`preview_paper` 執行前先驗章名並給候選）；兩者的 cassette 待 Owner 以本機模型重錄（`nlq` suite 8 句 LLM 路徑；助教沒有入庫的 cassette）。細節見 `docs/chemistry.md` 第 7、7.1 節。〔整合 2026-09-26〕更正：要重錄的只有 `nlq.v2`（LLM 路徑 8 句）與 CR-9 的 `classify.v2`；`assistant.v2` 不必重錄（`eval/cassettes/` 底下沒有 `assistant`，CI 也不回放助教），上面「兩者的 cassette 需重錄」的「兩者」只剩 NLQ | 第 1.1 條：既有 agent 的 SYSTEM、模板、schema 一個字都不能改（〔修訂 2026-09-26 合併回填〕Owner 以 B5 改判，接受改模板並重錄） | WS-B |
...
| S5-17 | NLQ 的化學分流 | 規則抓不到章節時：句子有化學線索**而且沒有任何數理線索**才跳過 LLM、`subject` 設化學；有數理線索（點名科目、含化學線索字的數理用語、既有數理詞典）照舊走 LLM；拿不準就走 LLM。〔修訂 2026-09-26 合併回填 B5〕S5-13 改判支援化學後（已合入 `local/integration` `7dc14a0`），化學句子**不再跳過 LLM**；「只查化學」的判斷改名 `chemistrySubjectPrior`（`isChemistryOnlyQuery`，或新的 `namesChemistry`：點名「化學」、「化學能」不算、沒點名數學／物理），只當**退路**——LLM 失敗或 LLM 科目與合法章節都沒給時才把 `subject` 補成化學；數理句子行為不變。本機模式在 `NLQ_TIMEOUT_MS` 預設 4 秒下，這類化學句子會多等 4 秒並附逾時警告（`docs/chemistry.md` 第 7 節） | 審查發現原版會把寫了「物理」的句子鎖進化學（數理 NLQ 回歸，已修）；把數理句子鎖進化學的代價大於化學句子查得較散 | WS-B |
...
| S5-28 | confirm-paper 不驗承上組 | 維持第 4.4 條「沿用既有 confirm-paper（不改）」：伺服器不重驗承上組是否完整，把關在前端（整組刪、整組加、確認前擋缺前題的承上題）；直接呼叫 API 仍可出半組。~~**待 Owner 確認**是否另開伺服器端檢查~~ 〔修訂 2026-09-26 決策單 B7〕Owner 2026-09-25 決策單：**改為伺服器端也檢查承上題整組**（直接呼叫 API 送半組也要擋）；由 `dec/b7-followup-server-check` 實作，合併前程式仍照本列原決定運作。〔修訂 2026-09-26 合併回填〕已合入 `local/integration`（`7dc14a0`）：`confirm-paper` 在「題目存在、未封存」之後查卷內每題所在承上組的全部成員（與補救卷加題同一段 `remedialService.buildItemLookupQuery`），半組回 400 並帶 `incomplete_groups`（缺題原因 `not_in_paper`／`archived`／`answered`），不寫卷、不寫 attempts、訊息不含學生姓名；順序不擋，一律重排。前端把關照舊。契約見 `docs/interfaces-stage1.md` 第 7.1 條 | 契約凍結 confirm-paper，既有程式刻意「照給的題出卷」（〔修訂 2026-09-26 合併回填〕Owner 以 B7 改判） | WS-D |
| S5-29 | 跨章配額（blueprint）規則 | 不掛旗標（核心組卷的延伸、不呼叫 LLM）；至少抽到一題就 200 並逐列回報不足，全部列都抽不到才 400；`FOLLOW_UP_SHORTFALL_POLICY` 延伸到 blueprint（`'error'` 時承上組不足的列回 400；〔修訂 2026-09-26 決策單 B10〕Owner 2026-09-25 決策單把承上題湊不滿的政策改為選項 B「直接報錯請老師改題數」，由 `dec/b10-shortfall-error` 實作，blueprint 的實際行為以該分支合併後為準；〔修訂 2026-09-26 合併回填〕已合入 `local/integration`（`7dc14a0`）：開關改為環境變數、預設 `'error'`，承上組不足的列讓整份 400 並逐列給建議題數（往上的建議不讓整張卷超過 50 題），且排在「全部列都抽不到」之前；`FOLLOW_UP_SHORTFALL_POLICY=note` 時與決策前逐字相同；補救卷草稿不受影響，見 `docs/remedial.md` 第 2.3 節）；單章路徑把 `chapter` 先過 pg `prepareValue` 再包成一元素陣列，非字串輸入維持原本的 400 | 契約只寫「逐列回報不足量」；單點政策開關應同時管兩條組卷路徑；候選池改為 `= ANY($2::text[])` 後直接包陣列，陣列輸入會變成多章卷或 500（審查 low，已修） | WS-D |
...
| S5-38 | 白名單外的內容與跨科先備 | 物理不另立「熱學」章（改 `LEGACY_CHAPTERS` 會讓全部 cassette 失效），只在「能量的形式與守恆」放一條概念性知識點，待 Owner 決定（〔修訂 2026-09-26 決策單 A6〕Owner 2026-09-25 決策單：白名單外內容——數學邏輯、空間向量加減、物理熱學——放進既有章當知識點，不新增章；放哪一章、寫哪幾條在第二輪知識點審定單待答。〔整合 2026-09-26〕第二輪知識點審定單已答（見 [`HANDOFF.md` §0.00a](HANDOFF.md#000a-owner-決策單第二三輪2026-09-26修訂-2026-09-26-決策單第二三輪)）：M4 邏輯兩條進「集合與計數原理」、M5 空間向量坐標運算進「空間概念與座標系」、P1 熱學兩條（比熱與熱平衡、潛熱與熱膨脹）進「理想氣體與氣體動力論」，仍不新增章；三科實作分支已合入 `dec/integration-r23`（待併 `local/integration`）。第二輪的 X1 也已答：只有 eval 分數可以紅燈合併，unit、integration、e2e 必須綠）；化學建議的 5 條跨科先備暫不寫入（等對方 code 定稿）；物理引用數學 5 章的 6 處跨科先備，整合時以不帶參數的 `validate_kc_seed` 三科一起驗證：637 個知識點、110 章、0 error（2026-09-24 文件整合時實跑；〔整合 2026-09-26〕`dec/integration-all` 實跑 `npm run kc:validate` 為 688 個、130 章、0 error） | 第 1.1 條；跨科代碼在單檔驗證時只給 warning，三科齊了才驗得了 | KC-P、KC-C、KC-M |
