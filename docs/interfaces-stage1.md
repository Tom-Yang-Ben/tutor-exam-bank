...
- **相鄰排列**：考卷排序以組為單位，依組首題的題型權重 → 難度排；組內依承接順序（組首 → 承上題深度優先，同一前題的多個承上題依 id 由小到大）緊接排列，不被題型權重拆開。`confirm-paper` 用同一排序，~~不重驗組是否完整（題目即預覽整組抽出的那批）~~。〔修訂 2026-09-26 合併回填 B7〕依 Owner 決策單 2026-09-25 B7（已合入 `local/integration` `7dc14a0`），`confirm-paper` 在「題目存在、未封存」之後**會驗組是否完整**：卷裡有某組的任一題、整組卻沒有全部在卷裡 → 400，不寫卷、不寫 attempts（格式見本條末）。順序不驗：一律照本條重排。
...
| 政策 `FOLLOW_UP_SHORTFALL_POLICY`（`controllers/examController.js`，~~**待 owner 決定**~~〔修訂 2026-09-26 合併回填 B10〕Owner 決策單 2026-09-25 B10 已決定：預設 `'error'`；改由環境變數切換） | 行為 |
|---|---|
| `'note'`（~~現行預設~~〔修訂 2026-09-26 合併回填 B10〕決策前的預設，設 `FOLLOW_UP_SHORTFALL_POLICY=note` 可切回，行為逐字不變） | 200，少出題；回應**額外**帶 `shortfall` 與 `note` 兩鍵（僅在少出時出現，湊滿時頂層鍵與本條原規格相同） |
| `'error'`（〔修訂 2026-09-26 合併回填 B10〕**預設**；未設、空白或非法值都當 `'error'`，非法值在伺服器 log 警告一次） | 400 ~~`承上題須與前題整組出題，無法剛好湊滿 ${N} 題（最多可出 ${actual} 題），請調整題數。`~~〔修訂 2026-09-26 合併回填 B10〕`承上題須與前題整組出題，「${chapter}」要 ${N} 題無法剛好湊滿（${N} 題以內最多只能湊到 ${actual} 題），請把題數改成 ${below} 題或 ${above} 題。`（`below`／`above` 是離 N 最近、剛好湊得滿的上下兩個題數，不超過 50 題；只有一個時只列一個，都沒有時為「請調整題數」）；不帶學生姓名 |
...
- 〔修訂 2026-09-26 合併回填 B10〕政策每次組卷時讀環境變數（改 `.env` 要重啟伺服器）；單章路徑、跨章配額 `blueprint`（`docs/remedial.md` 第 2.3 節，逐列訊息）與助教 `preview_paper`（回工具錯誤訊息）聽同一個開關。上表 `'note'` 列、`shortfall`／`note` 兩鍵的形狀，以及「庫存不足」「一題都湊不出來」兩條 400 都不變。
- 〔修訂 2026-09-26 合併回填 B7〕`confirm-paper` 的承上題整組檢查（`utils/followUpPaperCheck.js`；成員查詢與補救卷加題共用 `remedialService.buildItemLookupQuery`）：

  ```jsonc
  // 400：卷裡只放了一部分的承上組（組與組依組首 id 排序）
  {
    "message": "承上題必須與前題整組出卷，以下題組不完整：承上題組（#40、#41、#42）缺 #42。請把缺的題加回卷裡，或把整組刪掉後再確認。",
    "incomplete_groups": [
      { "group_ids": [40, 41, 42], "missing": [{ "question_id": 42, "reason": "not_in_paper" }] }
    ]
  }
  ```

  `reason`：`not_in_paper`（可以加回）、`archived`（已封存，訊息標「已封存」）、`answered`（該生已寫過，訊息標「該生已寫過」）；有後兩種時訊息多一句「已封存或該生已寫過的題不能出，含這種題的組只能整組刪。」。卷內本身有封存題時仍先回原本的 400 `部分題目已不存在或已封存，請重新預覽。`；沒有綁定的題（含題幹寫「承上題」但 `follows_question_id` 為 NULL 的舊題）自成一組、不檢查；200 的回應形狀不變。
