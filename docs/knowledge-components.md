...
- **已審定不覆寫、老師改過的草稿也不覆寫**：Owner 的審定是這套內容最貴的部分，載入腳本預設保護它；〔S5-43〕637 個知識點不可能一次審完，「改了、還沒審定」是正常會停留的狀態，章節切法等待決事項定案後種子檔勢必要更新重載，所以老師在分頁上改過的草稿（`edited_at`）一樣保護。`--force` 是明確的破窗，而且會逐條列出被覆寫的 code。
