...
## 2. 資料模型（migration `0012_knowledge_components.sql`，本分支沒有新增 migration）

| 表 | 重點 |
| :--- | :--- |
| `knowledge_components` | `code` 唯一（`MATH\|PHYS\|CHEM.<章名>.<兩位序號>`）；`status` 為 `draft`（AI 草稿）或 `approved`（Owner 審定）；`spoken_text` 是口語版；`sort` 是章內教學順序；`UNIQUE (subject, chapter, name)` |
...
| 2. 知識點 upsert | 以 code 比對。DB 沒有 → 新增；內容相同 → 略過；DB 是草稿 → 更新；DB **已審定** → 不覆寫（「已審定受保護」），加 `--force` 才覆寫（`--force` 連 `status` 也用種子檔的值） |
...
| 5. 回報 | 印出新增、更新、略過（內容相同／已審定受保護）、先備新增與移除；DB 有但種子檔沒有的知識點**不刪**（可能已有題目標到它），只提示數量 |
...
3. 按「朗讀」用耳朵聽一遍口語版（瀏覽器語音，zh-TW；再按一次停止）。判斷標準：
   ...
   - 數理化內容有沒有錯。寧可樸素，不可錯。
4. 不順就直接在卡片上改。口語版要 40–300 字、不能有 LaTeX（計數器變紅、寫著「有 LaTeX」就是不合格）。符號寫成唸得出來的樣子：a·b、|a|、cosθ、x²、H₂O、→。
5. 滿意就按「審定通過」——**會連同還沒儲存的修改一起送出**。卡片變成「已審定」。
6. 審定過的知識點，之後重新 `kc:load` 也不會被種子檔蓋掉（除非刻意加 `--force`）。想重寫就按「改回草稿」。
...
- **已審定不覆寫**：Owner 的審定是這套內容最貴的部分，載入腳本預設保護它；`--force` 是明確的破窗。
