...
>
> **狀態：2026-08-23 第一輪裁決全部下來，九條全數結案（裁決 S3-R17～R22，見
> `docs/interfaces-stage3.md` §15）。七條需要裁決的全部「採納現行做法」，程式碼因此
> 一行都不用改；只有 `utils/nlqHeuristics.js` 的檔頭把依據從「本檔第 1 題」改寫成
> 裁決 S3-R17 本身。本檔之後只當紀錄，不再是待辦清單。**
>
> 每一題原本都寫了「我這一版怎麼做」——那是為了讓 WS-C 能交付，不是既成事實；
> 裁決下來之後那些段落原文保留，後面各加一段「裁決」作結。

---

## 1.（**已結案 — 裁決 S3-R17**）`semantic_text` 的定義：第 6.1 條的文字與第 6／8.4 條的兩個範例互相矛盾
...
**裁決：S3-R17（2026-08-23）** —— 採納，以兩個範例為準：

> 「`semantic_text` 以第 6 條與第 8.4 條的兩個範例為準：概念詞（章節本名／別名）原文保留、
> 條件詞整段拿掉、自由文字剝頭尾虛詞；第 6.1 條那行散文改寫。」

§15 明寫「以下裁決**優先於上文對應條文**」，所以第 6.1 條那一行不再是依據。
落地：程式碼不變（本來就是這樣做的），`utils/nlqHeuristics.js` 的檔頭改成引用 S3-R17
當依據，不再指向本檔。**結案。**

---

## 2.（已結案 — 裁決 S3-R18）`buildHybridQuery` 沒有 `question_type` 參數，但第 6 條的 `filters` 有 `question_types`
...
**裁決：S3-R18（2026-08-23）** —— 「`question_types` 以 `excludeIds` 實作精確篩選（做法 b），
`buildHybridQuery` 簽名不動。」程式碼不變。**結案。**

---

## 3.（已結案 — 裁決 S3-R19）body 的 `student_id` 與句子裡解析出的「某某沒寫過」撞在一起時，誰優先？
...
原本問的另一個選項是「兩個都排除（取聯集）」，那要 `buildHybridQuery` 支援多個
`excludeStudentId`，一樣會動到凍結介面，所以我沒有走這條。

**裁決：S3-R19（2026-08-23）** —— 「句子裡指名的『X 沒寫過』優先於 body 的 `student_id`
（**不取聯集**）。」程式碼不變。**結案。**

---

## 4.（已結案 — 裁決 S3-R20、S3-R28）`--suite nlq` 的 LLM 欄在 cassette 錄好之前一定是 replay miss

> 原標題是「請 WS-D 先不要接進 CI」——**這個請求沒有被採納**：改成由開發者先錄 cassette，
> CI 兩個 suite 維持必跑。詳見下方裁決。
...
**裁決：S3-R20 + S3-R28（2026-08-23）** —— 「nlq 與 variant 的 cassette 由開發者在 main
統一錄（`LLM_MODE=record` + `EMBED_MODE=record` 一起開）；**CI 兩個 suite 維持必跑**。」
也就是說我請 WS-D 暫緩接 CI 的那個請求**不採納**——改成先把 cassette 補上，CI 照原計畫必跑。

S3-R28 記錄了錄製結果（2026-08-23 完成，commit `fd5cf5b`）：
nlq cassette 8 句 + 50 句查詢向量都已進 repo，rules 欄 coverage 0.84／filters_exact 1.0／
recall10 1.0，llm 欄 filters_exact 0.75／recall10 0.875。
同時 S3-R28 讓 `suiteNlq.js` 在 `EMBED_MODE=record` 時先補錄缺的查詢向量再量——
否則我寫的「先檢查、查不到就 n/a」會把 record 模式短路掉，查詢向量永遠錄不進去。
那一段由開發者直接落地，WS-C 不用動。

另外 S3-R28 附了一個觀察：LLM 路徑回的 `semantic_text` 多是關鍵詞形式
（「斜面 物體受力平衡」），與 golden 的整句期望不同；那只是警告，
定案 golden 時再決定 llm 路徑的 `semantic_text` 要不要比對。**結案。**

---

## 5.（已結案 — 裁決 S3-R21）第 6.4 條的六個步驟照字面順序做會自相矛盾（小問題，我自行處理了）
...
**裁決：S3-R21（2026-08-23）** —— 「第 6.4 條的順序：先反推 `subject`（第 6 點）
再逐一驗章節（第 1 點）。」程式碼不變。**結案。**

---

## 6.（已結案 — 裁決 S3-R21）`keywords` 要不要含題型？（很小，兩邊都不影響分數）
...
**裁決：S3-R21（2026-08-23）** —— 「`keywords` **含題型**（散文為準，前端 chip 由 WS-D
決定是否顯示）。」也就是後端固定含題型，要不要顯示是前端的事。程式碼不變。**結案。**

---

## 7.（已結案 — 裁決 S3-R22）level 3 的 LIKE：第 6.6 條寫的是單一 `$n`，但 `semantic_text` 是以空白分段的
...
**裁決：S3-R22（2026-08-23）** —— 「level 3 的 LIKE：`semantic_text` 逐段 ILIKE 以 OR 連接
（最多 5 段）；放寬順序＝先丟 ILIKE 留 metadata、再丟章節（**第 6.6 條 level 3 改寫**）。」
兩點都採納，第 6.6 條的條文以本裁決為準。程式碼不變。

補一個數字：上面那個 `recall10_like_only = 0.04` 是**改之前**量到的；
把放寬順序改成「先丟 ILIKE 留 metadata」之後同一份 golden 量到 **0.86**。
（cassette 與查詢向量錄好之後，真正的 `recall10` 是 rules 1.0／llm 0.875，見第 4 題。）
**結案。**

---

## 8.（知會事項，無需裁決；相關裁決 S3-R23、S3-R25）前端相關（我不能改 `public/**`，寫給 WS-D）
...
- `filters` 的八個鍵一律出現，可以直接拿去回寫下拉；`keywords` **固定含題型**
  （裁決 S3-R21；要不要在 chip 上顯示由 WS-D 決定，見第 6 題）。

**結案**：這四點是知會不是提問，不需要裁決。WS-D 的前端在第一輪已合入
（`b64f149`），相關裁決是 S3-R23（兩個 `CustomEvent`、`#nlq` 的位置）與
S3-R25（新增第四個注入點 `<meta name="feature-similar">`）。
...
**結案**：知會事項，不需要裁決；第一輪合併後仍然成立，WS-C 沒有新增任何環境變數。
