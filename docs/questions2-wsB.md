...
> 分支 `ws2-b/llm-agents`，2026-08-22。對應 `docs/interfaces-stage2.md` 第 3、5 條與 `docs/interfaces.md` 第 4 條。
>
> **`docs/interfaces.md` 與 `docs/interfaces-stage2.md` 一個字都沒有改。** 下面每一條都附上「我暫時怎麼做」——
> 都是在不動凍結簽名的前提下走得通的版本，等開發者本人裁決後再對齊。
> 標 🔴 的是會影響其他 workstream 的，請優先看。
...
## 附帶回報：兩題分類與公開 fixture 標註不一致（給人工 lane 定案）
