...
> 版本 v1.1 | 2026-09-15 | 分支 `feat/source-check` | 對應：`docs/roadmap-plan.md` §6.5 待辦 12、ADR-009、FR-020、DEC-013、`docs/interfaces-stage2.md` 裁決 S2-31
> 🛠 **v1.1（2026-09-15，程式碼審查修正）**：§3 第 5 步段落邊界兩條規則收緊（行首小數不算下一題題號；往回找題號不吞上一題結尾的小數）；以同批真實原卷重跑校準，可比對 63、TP 3、FP 0、樣卷誤報 0，與 v1.0 相同。
...
5. 段落從起點往前吞掉題號（題號前須為行首或非數字、題號到題幹之間不得再有數字，避免把上一題結尾的 `1.5 cm` 吞進來），往後延伸到最後一個中文字之後最多 600 字（`tailMax`），遇到「換行＋題號」（點後緊接數字者如行首的 `0.5` 是小數、不算題號——矩陣題文字層一格一行）「題組」「答案卷／答案欄」即截斷；截斷上限 1500 字（`segmentMax`）。
