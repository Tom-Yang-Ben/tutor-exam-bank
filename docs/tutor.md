...
| 數學式的 `\href` 等指令（**全站既有風險，交給整合階段**） | 家教回覆、語音歧義 chip、公式預覽會把受 LLM 影響的文字交給 `renderMath`；`renderMarkdown` 本身不產生連結，數學式片段依設計原樣留給 MathJax | `index.html` 的 MathJax 設定只有 `tex.inlineMath／displayMath`，沒有載入 `ui/safe`；MathJax 3 的 autoload 會在遇到 `\href` 時載入 html 擴充，`$\href{javascript:…}{x}$` 有機會變成可點的 `javascript:` 連結（審查者與 WS-E 都**沒有**在瀏覽器實測點擊）。題幹來自上傳的 PDF，有 prompt injection 的可能。題庫、試卷等既有頁面同樣走 `renderMath`，所以不是 WS-E 引入的，也不該只在家教頁修。修法見第 8 節 |
...
- **MathJax 安全設定（建議整合時處理，需要瀏覽器實測）**：`public/index.html` 的 `window.MathJax` 加上 `loader: { load: ['ui/safe'] }`（預設只允許 http／https／file 協定的連結，並過濾 `\class`、`\style`、`\cssId`）。另一個做法是 `tex: { packages: { '[-]': ['html'] } }` 並把 autoload 的 html 項目設成空陣列，但 `\require{html}` 仍可能把它載回來，所以優先用 `ui/safe`。改完用 `$\href{javascript:alert(1)}{x}$` 在家教回覆、題庫預覽各試一次，確認連結被拿掉、其他公式照常排版。這是全站設定，影響所有分頁，WS-E 依第 1.5 條不在自己的分支上改。
