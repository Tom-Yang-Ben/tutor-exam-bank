...
- **recall 失敗的 2 句都是過濾造成的，不是檢索**：fixture 每章最多 8 題、hybrid 兩側各取前 50，章節與題型對了，候選集 ≤ 10 題，相關題一定在前 10。
...
1. **伺服器端的證據檢查**（`services/nlqService.js` 的 `mergeLlm` 帶原句；判斷在 `utils/nlqHeuristics.js`）。規則沒抓到的條件，LLM 給的值要在句子裡找得到證據才採用：
   - 題型：句子要有題型字眼（`QUESTION_TYPE_CUES`：選、填、計算、證、問答、申論、論述、簡答、是非、應用題、題型）。「算」刻意不收：「怎麼算」「求…」「是多少」是題目在問的東西，不是指定題型。
   - 難度：句子要有難度字眼（`DIFFICULTY_CUES`：難、易、簡單、基礎、進階、挑戰、星、級…）。
...
2. **平面／空間對齊**（`alignChapterDimension`）：句子**明講**平面（「平面上」「坐標平面」「二維」…）或空間（「空間」「三維」「z 軸」…）時，
   LLM 挑到另一個維度的章就換成同名的那一章。對照表由 `CHAPTERS` 算（「空間＋X」而且 X 也是數學章節）：
   目前是〔空間向量內積 ↔ 向量內積〕〔空間直線方程式 ↔ 直線方程式〕。只有「平面」兩個字不算線索——「平面方程式」「過三點的平面」是空間單元。
...
3. **模板**（沿用 B5 已升的 `nlq.v2`，同一個未發布版本內改，不另升版）：
   ...
   - 新增【容易混淆的章】：平面／空間向量、指數與對數（第一冊／第三冊）、直角三角形／廣義角、運動學／受力（直線運動、平面運動／牛頓運動定律、摩擦力與向心力、剛體轉動與平衡）、
     靜電學／電場與電位。界線取自 `config/kc/*.json` 的知識點名稱與 `config/chapterAliases.js` 的別名歸屬（例如「斜面上的受力分析」是〔摩擦力與向心力〕的知識點、「斜面滑動」是它的別名）；
...
- **nlq-036**：規則 6 要模型通常只挑一章。只回〔摩擦力與向心力〕→ recall 中、filters_exact 不中；只回〔牛頓運動定律〕→ 兩者都不中；兩章都回 → 兩者都中（候選 14 題，recall 要看排序）。
  這句的計分語意等 Owner 看重錄結果再決定（A3），本輪不動它的 golden。
- **過度貼合的風險**：【容易混淆的章】是看了這 8 句的錯誤之後寫的，雖然內容取自知識點與別名資料、沒有照抄 golden 句子，
  重錄後在這 8 句上的分數仍可能高估對新句子的效果。建議之後另補幾句新的 LLM 路徑查詢當檢驗（要改 golden，屬 Owner 決定）。
- **prompt 變長**：模板本體 391 → 1,642 字，含白名單的整段 prompt 約 1,435 → 2,686 字。本機模型的 prefill 會變慢（沒有量）；
...
- `test/unit/nlqLlmMerge.test.js`（24 項）：證據字、平面／空間線索與對照表、`mergeLlm` 帶原句與不帶原句（不帶時行為與改版前相同）、
  `parseOnly` 的 `adjustments`（不產生新 warning、快取帶回）、【容易混淆的章】的章名都在白名單、【範例】合 schema 且規則抓不到章節、
  模板與 schema 不含 golden 查詢原句、`keywords` 排第一。
