...
| 階段 | 步驟 | 觸發 |
| :--- | :--- | :--- |
| 單元層 | `npm audit --omit=dev --audit-level=high` ＋ `npm test`（1,507 項）＋ `npm run check:html`，Node 22.x／24.x 矩陣〔修訂 2026-09-15f〕 | 每次 push 與 PR（GitHub Actions） |
| 整合層 | 起 `pgvector/pgvector:pg16` service → 整合 290 項〔修訂 2026-09-15f〕＋e2e 11 項＋五個 eval suite（ratchet 門檻）〔修訂 2026-08-29〕 | 同上，`integration` job |
