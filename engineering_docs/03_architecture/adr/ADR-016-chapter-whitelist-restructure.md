...
- 新章沿用舊名的拆分（排列、組合…共 9 章）讓「還沒處理」與「老師確認留在原章」在 `questions.chapter` 上無法區分，所以新增 `chapter_migration_log`（migration 0014）當作已處理的紀錄與稽核軌跡，重跑 `--dry-run` 才不會把確認過的題再提議一次。紀錄同時是 `--apply` 的冪等依據：**已記錄的題一律不再改**（同一天的第二份提議檔、或搬完後老師又在題庫頁改回沿用舊名的章，都略過並列出），以第一次套用與老師之後的手動修改為準。同樣因為沿用舊名，重整後 AI 以新白名單分到這 9 章的新題也長得跟舊題一樣，所以 `--dry-run` 以 0014 的套用時間（上線那一步）為截點，這 9 章只列截點之前入庫的題（`--include-new` 取消截點）。
