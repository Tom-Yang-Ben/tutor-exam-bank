...
> 🛠 **2026-09-26 整合**（分支 `dec/integration-r23`）：FR-019 列的模組路徑補 `utils/followUpPaperCheck.js`（B7 確認出卷時伺服器端檢查承上題整組）、驗證補單元 `followUpPaperCheck.test.js`（B7）、`followUpShortfallPolicy.test.js`（B10 承上題湊不滿預設報錯）；B7、B10 已在 `local/integration`（`7dc14a0`）。全域測試數（NFR-003）等錯題重練合入後統一更新，本次不動。修改處以〔整合 2026-09-26〕行內標記。
