...
> 🛠 **2026-09-26 整合**（分支 `dec/integration-r23`）：FR-019 列的模組路徑補 `utils/followUpPaperCheck.js`（B7 確認出卷時伺服器端檢查承上題整組）、驗證補單元 `followUpPaperCheck.test.js`（B7）、`followUpShortfallPolicy.test.js`（B10 承上題湊不滿預設報錯）；B7、B10 已在 `local/integration`（`7dc14a0`）。全域測試數（NFR-003）~~等錯題重練合入後統一更新，本次不動~~（〔整合 2026-09-26〕已在 `dec/integration-all` 統一更新，見下一列）。修改處以〔整合 2026-09-26〕行內標記。
> 🛠 **2026-09-26 整合之二**（分支 `dec/integration-all`＝`dec/integration-final`＋錯題重練 `dec/retrain-phase2-fix`＋本機看圖逾時 `dec/local-vision-timeout`）：§1 新增 FR-036～040 五列（模組與測試檔以實際檔案為準）、FR-028 知識點數 637 → 688；§2 NFR-003 測試數更新為本版實測（上一列「等錯題重練合入後統一更新」已完成）、NFR-006 補 0013–0018 與「派題與作答同一交易、批改與排程重算同一交易」、新增 NFR-010；§3 ADR 索引補 ADR-018、ADR-019（v1.1）；§4 追溯。修改處以〔整合 2026-09-26〕行內標記。
...
- [3. ADR 索引](#3-adr-索引)
- [4. 追溯](#4-追溯)
- [5. 相依與平行開發](#5-相依與平行開發修訂-2026-08-29-新增)〔修訂 2026-08-29〕

## 1. 功能需求 FR

全部狀態為「已實作」（四階段完成，CI 全綠 @ f8f6574〔修訂 2026-08-29〕）；FR-021～035 為階段 5，實作於整合分支 `stage5/integration`（@ `6f8e671`，完整 `ci.sh` 全綠，主控實跑），尚未併入 main〔修訂 2026-09-24〕。〔整合 2026-09-26〕FR-036～040 為錯題重練與間隔複習，實作於 `dec/integration-all`（完整 `ci.sh`：unit、check:html、migrate、integration 全綠），待併 `local/integration`。模組路徑為 repo 相對路徑。
...
| FR-036 | 派題與作答拆表：`assignments`（new／retrain）＋`attempt_records`，`attempts` 改為唯讀相容檢視（每生每題第一次＝拆表前語意），新題每生每題一次由部分唯一索引硬擋〔整合 2026-09-26〕 | 已實作〔整合 2026-09-26〕（`dec/integration-all`，待併 `local/integration`；核心資料層，不受旗標管） | exam_pro/migrations/0016_assignment_attempt_split.sql、exam_pro/controllers/examController.js（writePaper、deletePaper）、exam_pro/controllers/paperController.js（GET／PATCH 改讀寫 `assignment_attempts`／`attempt_records`）、exam_pro/controllers/studentController.js、exam_pro/controllers/studentAdminController.js（刪學生、合併）、exam_pro/controllers/questionController.js（刪題保護改讀 `assignments`）、exam_pro/services/assistantService.js（list_students）、exam_pro/eval/lib/pgEngine.js（TRUNCATE）、exam_pro/scripts/snapshot_attempt_views.js、exam_pro/test/helpers/attempts.js；一次性切換工具 exam_pro/migrate/{import_pg,export_pg_delta,verify}.js 檔頭註明只適用 0016 之前 | ADR-018 | 單元（assignmentSplit.test.js、noWritesToAttemptsView.test.js）＋整合（assignmentSplit.pg.test.js、assignmentWrites.pg.test.js、schema.test.js 三條依決策改寫）＋既有整合與 e2e 全部（斷言不改） |
| FR-037 | 錯題重練清單：批改卡勾「要重練」才進（R1 選 2）、承上組整組、手動加入／移出／判定已會／重新加入、不補建舊錯題（R11 選 3）〔整合 2026-09-26〕 | 已實作〔整合 2026-09-26〕（`dec/integration-all`，待併 `local/integration`；旗標 `FEATURE_RETRAIN` 預設關） | exam_pro/migrations/0017_retrain_items.sql、exam_pro/migrations/0018_retrain_unflag_marker.sql、exam_pro/services/retrainService.js、exam_pro/controllers/retrainController.js、exam_pro/controllers/paperController.js（API-9、API-10）、exam_pro/utils/retrainValidation.js、exam_pro/routes/index.js（檔尾錯題重練區塊）、exam_pro/config/features.js（FEATURE_RETRAIN）、exam_pro/app.js（`__FEATURE_RETRAIN__`、`retrain-attach-ratio` meta）、exam_pro/public/js/students.js（批改卡勾選框與徽章、學生清單徽章）、exam_pro/public/js/retrain.js（錯題重練卡） | ADR-018、ADR-019 | 單元（retrainValidation、retrainService、retrainUi、retrainContract）＋整合（retrain.pg、retrainFlow.pg、retrainMigration.pg） |
| FR-038 | 間隔複習排程：固定關卡 1／7／14 天、連對 3 次練到會、錯滿 3 次卡關、只有全對算對；排程是作答歷史的純函式，改判／取消批改／刪卷後重算〔整合 2026-09-26〕 | 已實作〔整合 2026-09-26〕（`dec/integration-all`，待併 `local/integration`；旗標 `FEATURE_RETRAIN` 預設關） | exam_pro/services/retrainSchedule.js（純函式）、exam_pro/config/retrain.js（`RETRAIN_*`）、exam_pro/services/retrainService.js（重算與快取寫回）、exam_pro/scripts/recompute_retrain.js（`npm run retrain:recompute`） | ADR-019 | 單元（retrainSchedule、retrainScheduleProperty、retrainScheduleCutoff、retrainConfig）＋整合（retrain.pg：每一步快取＝純函式結果、重算失敗整筆回滾、併發） |
| FR-039 | 出卷帶入重練題：新卷附上到期的重練題（預設新題數三成）、獨立重練卷、補救卷重練組；承上組整組、放不下報錯（R12 選 2）；重練題不佔家族名額；Word 答案區標「（重練）」〔整合 2026-09-26〕 | 已實作〔整合 2026-09-26〕（`dec/integration-all`，待併 `local/integration`；旗標 `FEATURE_RETRAIN` 預設關） | exam_pro/services/retrainSelect.js、exam_pro/controllers/retrainController.js（API-5）、exam_pro/controllers/examController.js（attachRetrain、writePaper 的 `retrainQuestionIds`、刪卷的 409）、exam_pro/controllers/remedialController.js（`retrain_count`）、exam_pro/controllers/wordController.js、exam_pro/services/wordService.js（answerHeading）、exam_pro/public/index.html（組卷頁〔retrain〕掛鉤）、exam_pro/public/js/remedial.js、exam_pro/public/js/retrain.js | ADR-019 | 單元（retrainSelect、retrainValidation、solutionText、retrainPaperUi）＋整合（retrainPaper.pg、retrain.pg、retrainFlow.pg）＋e2e（paperWord.e2e.test.js 新增一案） |
| FR-040 | 重練成效與到期提醒：成效表（重練與回測答對率、練到會、卡關、逾期量）、學生清單到期數、批改卡與試卷明細標示；弱點與補救卷只算第一次作答（R10 選 1）〔整合 2026-09-26〕 | 已實作〔整合 2026-09-26〕（`dec/integration-all`，待併 `local/integration`；旗標 `FEATURE_RETRAIN` 預設關） | exam_pro/services/retrainStatsService.js、exam_pro/controllers/retrainController.js（API-4 summary、API-13 stats）、exam_pro/public/js/retrain.js（重練成效小表）、exam_pro/public/js/students.js（學生清單徽章）；弱點與補救卷照舊讀相容檢視 `attempts`（weaknessService、kcWeaknessService、remedialService 未改） | ADR-018 | 單元（retrainStats、retrainUi）＋整合（retrainStats.pg、retrain.pg 的 API-4、retrainPaper.pg 的 ACPT-040-3） |
...
| NFR-006 | 資料一致性：組卷與作答歷史同交易；migrations 只增不改（0001–0009〔修訂 2026-09-15e〕〔修訂 2026-09-15f〕；0010–0012〔修訂 2026-09-24〕；〔整合 2026-09-26〕0013–0018）；〔整合 2026-09-26〕派題與作答同一交易寫入、批改與排程重算同一交易 | 已實作 | exam_pro/controllers/examController.js、exam_pro/migrations/、exam_pro/migrate.js、〔整合 2026-09-26〕exam_pro/services/retrainService.js（重算收已 BEGIN 的 client）、exam_pro/scripts/snapshot_attempt_views.js | ADR-001、ADR-018 | 整合（〔整合 2026-09-26〕assignmentSplit.pg、retrain.pg 的「重算失敗整筆回滾」） |
...
| NFR-010 | 效能（錯題重練，草案）：單一學生 5,000 筆派題、題庫 5,000 題時 API-1、API-5 本機 < 300 ms；候選池 EXPLAIN 走部分唯一索引〔整合 2026-09-26〕 | 已實作（`dec/integration-all`）；只量過一次 | exam_pro/services/retrainService.js、exam_pro/services/retrainSelect.js、exam_pro/services/retrainStatsService.js、exam_pro/migrations/0016_assignment_attempt_split.sql（索引） | ADR-018 | 一次性手動量測（中位數 API-1 74 ms、API-5 63 ms、API-13 61 ms，`docs/retrain-and-review.md` 第 5.6.5 節，未進 CI）＋整合（assignmentSplit.pg 的 EXPLAIN，TC-036-4） |
...
| ADR-018-assignment-attempt-split-compat-view〔整合 2026-09-26〕 | 狀態：已接受（v1.0）。DEC-003 例外條款選 a：`attempts` 拆成派題 `assignments`（new／retrain）＋作答 `attempt_records`，`attempts` 名字留給唯讀相容檢視（每生每題第一次＝拆表前語意）；新題每生每題一次仍由資料庫硬擋（部分唯一索引）；沒有重練資料時既有讀法、數字與回應逐字不變 |
| ADR-019-leitner-fixed-steps-pure-schedule〔整合 2026-09-26〕 | 狀態：已接受（v1.1：補「已派出」的範圍與重新加入的同日邊界、測試說明如實化；決策不變）。家教一週一兩次、資料少：固定關卡（1／7／14 天、連對 3 次練到會、錯滿 3 次卡關）比 SM-2／FSRS 好懂、不需校準；排程是作答歷史的純函式，`retrain_items` 只是快取，改判／刪卷／合併後重算結果與從頭批改相同 |

## 4. 追溯

- 上游：DEC-001～012（[requirements_tracker](../01_requirements/requirements_tracker.md) §1；DEC-010／011 為 2026-08-29 補登錄〔修訂 2026-08-29〕；DEC-012 為 2026-09-15 新增〔修訂 2026-09-15e〕）、DEC-013〔修訂 2026-09-15f〕、DEC-014～019〔修訂 2026-09-24〕、DEC-003 例外條款與 DEC-016 的錯題重練（設計 `docs/retrain-and-review.md`，Owner 決策單第三輪 R1～R12）〔整合 2026-09-26〕；凍結介面與裁決（`docs/interfaces*.md`；原卷比對為裁決 S2-31；階段 5 為裁決 S5-1～S5-39〔修訂 2026-09-24〕）。
- 下游：FR-001～020 → TC-＊與五個 eval suite（[qa_tracker](../05_qa/qa_tracker.md)）〔修訂 2026-09-15e〕〔修訂 2026-09-15f〕；FR-021～035 → TC-021-*～TC-035-*〔修訂 2026-09-24〕；FR-036～040 → TC-036-*～TC-040-*〔整合 2026-09-26〕；FR-010～012、FR-016 → 各 ui_spec（[../02_ux_ui/](../02_ux_ui/)）；FR-021～023 → ui_spec-students、FR-028～029 → ui_spec-kc、FR-030～033 → ui_spec-remedial、FR-034～035 → ui_spec-tutor〔修訂 2026-09-24〕；NFR-005、DEC-004 → runbook（[../06_ops/](../06_ops/)）；NFR-007 → runbook-llm-cost-quota〔修訂 2026-09-24〕。
