...
> 🛠 **2026-09-15d 修訂**（測試數同步）：SCN-005 單元測試現況 1,445→1,476（main f2af3c2 實測，2026-09-15 晚間）；§4 Gate 核准證據維持當時數字不改。修改處以〔修訂 2026-09-15d〕行內標記。
> 🛠 **2026-09-15f 修訂**（feat/follow-up-links 測試數同步）：SCN-005 單元測試現況 1,476→1,507（本分支實跑）。修改處以〔修訂 2026-09-15f〕行內標記。
> 🛠 **2026-09-16b 修訂**（主線同步，PR #30–#33 合併後）：SCN-005 單元測試現況同步為 1,613 passed（PR #30–#33 併入 main 後 CI 實測）。修改處以〔修訂 2026-09-16b〕行內標記。
...
| ID | 業務情境 | 通過準則（可觀察） | 對應 FR／ACPT |
| :--- | :--- | :--- | :--- |
| SCN-005 | `npm test` 單元測試 | 全數通過（2026-09-16 現況 1,613 passed / 0 failed，PR #30–#33 併入 main 後 CI 實測）〔修訂 2026-09-16b〕；不連網、不連庫、零 secrets | NFR-003、NFR-004 |
