...
| **測試層級** | 三層：單元 **1,507** 項（`test/unit/`，node:test，無 I/O）／整合 **290** 項〔修訂 2026-09-15f〕（`test/integration/`，對 `_test` 後綴 PG）／e2e **11** 項（`test/e2e/`，經 HTTP 走真 runner）；另有五個 eval suite（§2）〔修訂 2026-08-29〕 |
...
| 案例與證據 | TC-* 維護於 [`qa_tracker.md`](./qa_tracker.md) ①測試設計；執行證據（1,507／290／11〔修訂 2026-09-15f〕、五個 suite、CI badge）於 ②執行證據〔修訂 2026-08-29〕 |
