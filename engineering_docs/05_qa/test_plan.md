...
| **測試層級** | 三層：單元 **1,613** 項（`test/unit/`，node:test，無 I/O）／整合 **317** 項〔修訂 2026-09-16b〕（`test/integration/`，對 `_test` 後綴 PG）／e2e **11** 項（`test/e2e/`，經 HTTP 走真 runner）；另有五個 eval suite（§2）〔修訂 2026-08-29〕 |
