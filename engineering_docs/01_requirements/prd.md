...
| FR-019 | 承上題綁定（DEC-012）〔修訂 2026-09-15e〕 | ACPT-019-1 | Given 題幹任何位置出現「承上題」（含「承上一題」與簡體寫法），When 該題入庫，Then 綁到同份考卷上一題最後落在的題目——上一題入新題、與題庫既有題相同、與同卷較早題相同、語意重複時，分別解析到實際題號（已驗證） |
| | | ACPT-019-2 | Given 上一題仍待複核，Then 承上題暫不綁定；老師核准或合併上一題後自動補綁或改綁；上一題被「不採用」且非重複題時不綁定（已驗證） |
| | | ACPT-019-3 | Then 人工指定的綁定不被自動流程覆寫；兩題學科不同、綁定會成環、上一題無法確定（拆題空號、全卷第一題）時不綁定並回報原因；複核畫面顯示承上題的前題與未綁原因（已驗證） |
| | | ACPT-019-4 | When 執行回填，Then 舊題透過重拆紀錄補綁（只補空、不覆寫）；沒有任何拆題紀錄可推前題的承上題列為待人工綁定，不猜測（已驗證） |
| | | ACPT-019-5 | When 組卷抽到承上題或其前題，Then 整組一起出現且順序正確（待 PR2） |
| | | ACPT-019-6 | When 刪除或封存被承接的前題，Then 系統擋下或提示整組處理，不留下缺前情的承上題（待 PR3） |
