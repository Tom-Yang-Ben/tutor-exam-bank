...
| | | ACPT-019-7 | Given 承上組內有任一題不能出給這位學生（已寫過、老師按了換題、已封存、題源不符、分在別的章節），When 組卷，Then 整組都不抽，不出現缺前題的承上題；Given 剩下的名額塞不下下一整組，Then 少出題並在預覽與出卷結果上告訴老師實際題數與原因（政策待 Owner 決定：少出題附註或直接擋下請老師改題數）；一題都湊不出來時擋下並提示調高題數（已驗證，同上〔修訂 2026-09-15g〕） |
