...
> **實例:** 單例
> **定位:** 本文件回答「系統提供哪些功能（FR-001～018、FR-020〔修訂 2026-09-15f〕）、各功能以何種可觀察行為驗收（ACPT）、AI 不確定性的邊界場景（SCN）」；需求決策沿革歸 [`requirements_tracker.md`](./requirements_tracker.md)，技術實作與 ADR 歸 `../03_architecture/`。〔修訂 2026-08-29〕
...
| 項目 | 內容 |
| :--- | :--- |
| **功能範圍** | 拆題管線（FR-001～006）／題庫與出卷（FR-007～009）／RAG 三落點與 NLQ（FR-010～013）／產品收斂與助教（FR-014～016）／上線後增量：來源標記與附圖（FR-017～018）〔修訂 2026-08-29〕、拆題結果對照原卷文字層（FR-020）〔修訂 2026-09-15f〕 |
...
| **資料界線** | 題庫屬私有資產（DEC-009）：資料與驗證邏輯留本地，僅 LLM 呼叫對外；學生姓名不出境：NLQ 與助教送 LLM／embedding 前以 `exam_pro/utils/pseudonym.js` 換成「學生#<id>」代號，回覆後還原〔修訂 2026-09-15b〕；repo 不含題庫內容，示範題自製（`exam_pro/seed_questions.js` 30 題） |
...
| 拆題結果對照原卷文字層〔修訂 2026-09-15f〕 | 2026-09-15 原卷核對發現拆題抄錯題幹多於抄錯答案、verify 不比題幹；決定性文字層比對，預設 enforce（ADR-009，權威文件 `docs/source-check.md`），正規化為 FR-020／DEC-013。已知限制：約一半題目跳過比對（掃描檔、中文字太少、定位不到），抓不到「選項整排前移」這類結構性錯誤 | 已實作（feat/source-check）；DEC-013 待 Owner 簽核 | Ben |
...
| 項目 | ID |
| :--- | :--- |
| 上游 | DEC-001～DEC-011（[`requirements_tracker.md`](./requirements_tracker.md) ①需求決策）〔修訂 2026-08-29〕、DEC-013〔修訂 2026-09-15f〕 |
| 本文件產出 | FR-001～FR-018、ACPT-001-1～ACPT-018-2、SCN-011～SCN-016〔修訂 2026-08-29〕；FR-020、ACPT-020-1～ACPT-020-3〔修訂 2026-09-15f〕 |
