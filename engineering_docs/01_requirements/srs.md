...
> **實例:** 單例（整個系統一份）
> **定位:** 本文件將 19 條功能需求（FR-001～018、FR-020〔修訂 2026-09-15f〕）對應至 API 端點與 FEATURE_* 旗標，並將 NFR-001–006 逐條可驗證化（量測值與門檻）；需求決策沿革歸 [`requirements_tracker.md`](./requirements_tracker.md)，架構取捨歸 [`../03_architecture/adr/`](../03_architecture/adr/)。〔修訂 2026-08-29〕
...
| NFR-003 | 可測試性 | agent 為純函式合約（不碰 DB、不讀 env、ctx 注入）；LLM 呼叫走 cassette record/replay；CI 零金鑰、零網路、零成本 | 單元 1,534／整合 269／e2e 11 全數通過（feat/source-check 實測，2026-09-15）〔修訂 2026-09-15f〕；CI 無 GEMINI_API_KEY | node:test＋cassette 重播（CI） |
...
| NFR-006 | 資料一致性 | confirm-paper 之組卷與作答歷史（attempts）寫入同一交易；migrations 只增不改 | migrations 0001_init–0007_source_detail 與 0009_source_check，共 8 份（0008 預留給承上題綁定分支），無修改既有檔〔修訂 2026-09-15f〕 | 整合測試＋migration 檔案稽核 |
...
| 項目 | ID |
| :--- | :--- |
| 上游 | DEC-001–DEC-011（[`requirements_tracker.md`](./requirements_tracker.md)）、[`prd.md`](./prd.md) 之 FR 初稿〔修訂 2026-08-29〕；DEC-013〔修訂 2026-09-15f〕 |
| 本文件產出 | FR-001–FR-018、FR-020〔修訂 2026-09-15f〕、NFR-001–NFR-006、使用案例（§5.1，對應 UAT SCN-009／SCN-010）、ACPT 對照（§6）〔修訂 2026-08-29〕 |
