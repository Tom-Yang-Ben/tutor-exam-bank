...
**本 suite 刻意把輸入的 `chapter_confidence` 設成 0、`chapter` 給 golden 的 `decoy_chapter`
（不給正解），強迫每一筆都走第二層。** 理由：如果把正解連同高信心一起餵進去，
閘門會 100% 命中、accuracy 恆為 1.0，而第二層一次都不會被呼叫——
那量到的是「我把答案抄給它然後它抄回來」。零成本閘門的通過率改在 `--suite pipeline` 量
（那裡的信心值來自真的 extract 輸出）。這個約定已寫進 `docs/questions2-wsD.md` Q4 待裁決。
...
### 兩個 suite 現在的狀態：骨架就緒，數字還沒有

`agents/*`（WS-B／WS-C）與 `pipeline/stateMachine.js`（WS-A）尚未合入，所以：
...
- `ans-040`：`0.3333333333` vs `1/3` 差 > 1e-9，依容差應判 `disagree`——要不要放寬
- `ans-043`：`2.4e-4` 這種 e 記法支不支援
- `ans-046`：`^\circ` 與 `^{\circ}` 的等價
- **`extraction_hazard: true` 的 2 筆（`ans-017`、`ans-047`）**：見 `docs/questions2-wsD.md` Q3。
  第 4.2 條「取第一個 `$…# eval —— 評估體系（WS-D）
...
」對 fixture 45 題只抽對 4 題，這 2 筆的 `expect` 是照現行規則寫的；
  規則若改，這 2 筆要一起改
