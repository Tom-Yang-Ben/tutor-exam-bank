...
> 〔修訂 2026-09-26 決策單〕Owner 2026-09-25「出題系統決策單」第一輪已答第 6、8.6 節的大部分問題：#61、#62 題目與答案核可（A1）、#52 維持直線運動（A2）、retrieval 負樣本不動（A4）、第一冊只教常用對數（A5，第 6 節第 1、2 項維持現標）；nlq-036 的計分等重錄結果再定（A3，待決）。檔案內容都不必改。
...
- `retrieval.json`：只有題號，沒有章名，不動。hard_negatives 與 `_suggestion.basis` 當初依舊章取「同章干擾題」，改標後有幾筆已不再同章：
  ...
  要不要把 #61 加進 R017、R018 的 hard_negatives，留給 Owner 決定（第 6 節第 6 項）。〔修訂 2026-09-26 決策單 A4〕Owner 2026-09-25：不動。
  CR-8 之二之後同樣的情形還有 R031～R034（hard_negatives 的 #51）與 R035、R036（hard_negatives 的 #59；靜電學的干擾題是新增的 #62），同樣不動（第 8.6 節第 4 項；〔修訂 2026-09-26 決策單 A4〕Owner 2026-09-25 確認不動，#62 不加入）。
...
| 檔案 | 題號 | 新增到的章 | 理由 |
|---|---|---|---|
| questions.public.json | 61 | 直角三角形的邊角關係 | 自製干擾題（單選、難度 2、role=distractor）：直角三角形已知兩股求 sin A，考銳角三角比的定義；與 #25、#26（平方關係）同章不同概念。答案由 AI 撰寫，~~待 Owner 核對~~ 〔修訂 2026-09-26 決策單 A1〕Owner 2026-09-25 核可題目與答案 |
| classify.json | cls-fx-061 | 直角三角形的邊角關係 | fixture 段逐字沿用 fixture #61 的題幹與標籤（規劃 §5.3.2「標籤沿用 fixture」） |
| questions.public.json | 62 | 靜電學 | 〔CR-8 之二〕自製干擾題（單選、難度 2、role=distractor）：帶電棒靠近金屬球、接地後先拆接地線再移走帶電棒，問球的帶電情形，考靜電感應與感應起電（知識點 靜電學.03）；與 #54、#55（庫侖力計算）同章不同概念。答案由 AI 撰寫，~~待 Owner 核對~~ 〔修訂 2026-09-26 決策單 A1〕Owner 2026-09-25 核可題目與答案 |
...
1. **一般底數的對數（#1、#2、#7，cls-dr-001、cls-dr-020）**：如果 Ben 的龍騰第一冊已經教一般底數的對數律與換底公式，這幾題應該改回「指數與對數」。
   〔修訂 2026-09-26 決策單 A5〕Owner 2026-09-25：第一冊「指數與對數」只教常用對數，一般底數與換底在第三冊——維持現標（指數函數與對數函數），不改回。
2. **指數方程式（#3、#4，cls-dr-002）**：高一用指數律就解得出來。如果龍騰第一冊有這類題，改回「指數與對數」。
   〔修訂 2026-09-26 決策單 A5〕Owner 2026-09-25：指數方程式在第三冊——維持現標（指數函數與對數函數），不改回。
3. **平方關係（#25、#26，cls-dr-008、var-013、樣卷第 4 題）——已依題意改歸「直角三角形的邊角關係」**：
   ...
   現已改標，並新增自製干擾題 #61 讓 D-E1 成立（第 2.1 節）。~~**請 Owner 核對 #61 的題目與答案。**~~ 〔修訂 2026-09-26 決策單 A1〕Owner 2026-09-25 核可 #61 的題目與答案（照現稿用）。
...
6. **retrieval 的 R017、R018**：hard_negatives 仍是 #29、#30（改標後已不同章，但仍是負樣本）。要不要改列或加列新章的干擾題 #61，請 Owner 決定；本分支不動已定案的檢索 golden（第 1 節）。
   〔修訂 2026-09-26 決策單 A4〕Owner 2026-09-25：不動（不改列、不加 #61）。
...
> 狀態：改標依 Owner 裁決；新增的 #62 由 AI 撰寫，~~**題目與答案待 Owner 核對**~~ 〔修訂 2026-09-26 決策單 A1〕Owner 2026-09-25 決策單核可 #62 的題目與答案（照現稿用）。驗證：`test/unit/evalChapterRelabel.test.js` 逐列核對本節兩張表，並確認改標題的衍生項沒有漏列。
...
1. **#62 的題目與答案**（第 8.2 節）。〔修訂 2026-09-26 決策單 A1〕Owner 2026-09-25：題目與答案核可，照現稿用（連同 #61，第 6 節第 3 項）。
2. **nlq-036 的「放寬」在指標上的實際意思**：`filtersExact` 比的是整組章節（排序後逐字相同），不是「命中其中一章就算對」。
   ...
   〔修訂 2026-09-26 決策單 A3〕Owner 2026-09-25：重錄完看分數再決定——**仍待決**；在那之前 golden 與 `filtersExact` 都不改。
3. **#52（甲乙兩車的相對速度，填空）仍標直線運動**：它要計算，依第 8.0 節第三條不歸必修章；但知識點 PHYS.平面運動.03 是「相對運動」，別名「相對運動」也指向平面運動。
   ...
   〔修訂 2026-09-26 決策單 A2〕Owner 2026-09-25：#52 維持「直線運動」，不改標，也不必補干擾題。
4. **retrieval 的 R031～R034、R035、R036**：hard_negatives 分別是 #51、#52 與 #59；#51 已改到物體的運動、#59 在電場與電位（query #54、#55 已改到靜電學），
   ...
   〔修訂 2026-09-26 決策單 A4〕Owner 2026-09-25：R031～R036 都不動，#62 不加入 R035、R036。
