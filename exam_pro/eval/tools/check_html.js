...
function checkContracts() {
    ...
    for (const fn of ['apiFetch', 'showToast', 'renderMath', 'escapeHtml', 'createQuestionEditor']) {
        const bridge = html.match(/window\.ExamApp\s*=\s*Object\.assign\([\s\S]*?\);/);
        if (bridge && !bridge[0].includes(fn)) {
            problems.push(`window.ExamApp 的橋接少了 ${fn}（第 8 條列了五個）`);
        }
    }
    // 舊流程必須保留可用（第 8 條：既有測試是契約）
    for (const marker of ['/api/analyze-pdf', '/api/batch-save-questions']) {
        if (!html.includes(marker)) {
            problems.push(`public/index.html 找不到 ${marker}：舊流程必須保留可用（第 8 條）`);
        }
    }
    return problems;
}
...
function main() {
    ...
    if (all.length === 0) {
        console.log('✅ 全部通過（語法 + 第 8 條的三個接點）。');
        return;
    }
    console.error(`\n❌ 發現 ${all.length} 個問題：\n\n${all.join('\n\n')}`);
    process.exit(1);
...
module.exports = { checkAll, checkContracts, extractInlineScripts, stripHtmlComments, checkSyntax, PUBLIC_DIR };
