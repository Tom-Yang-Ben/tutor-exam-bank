...
// 〔retrain PR-4〕錯題重練卡（docs/retrain-and-review.md 第 5.3 節）：形狀同 STAGE5_PAGES，分開列是因為 STAGE5_PAGES 是凍結的清單。
const RETRAIN_PAGES = [
    { id: 'retrain', meta: 'feature-retrain', placeholder: '__FEATURE_RETRAIN__' }
];
// FEATURE_RETRAIN 也管學生分頁裡的東西（批改卡的「要重練」勾選框、學生清單的到期徽章）：students.js 必須讀同一個 meta。
const RETRAIN_EXTRA_METAS = [
    { meta: 'feature-retrain', placeholder: '__FEATURE_RETRAIN__', reader: 'public/js/students.js' }
];
...
function checkContracts() {
    ...
    for (const page of [...STAGE3_PAGES, ...STAGE4_PAGES, ...STAGE5_PAGES, ...RETRAIN_PAGES]) {
    ...
    for (const extra of [...STAGE3_EXTRA_METAS, ...STAGE5_EXTRA_METAS, ...RETRAIN_EXTRA_METAS]) {
...
module.exports = {
    ...
    RETRAIN_PAGES, RETRAIN_EXTRA_METAS
};
