...
const MATH_PHYSICS_TERMS = Object.freeze([
    ...
    '基本粒子', '夸克',
    // 〔章節重整 CH-A〕2026-09-25（docs/chapter-restructure.md 第 3.1 條第 4 點）：新章的名詞。
    // 新章名本身與「A與B」拆出的子詞由 expandChapterWords 自動進詞典，這裡只補章名以外、題幹常見的詞。
    // 只收數學／物理專用的詞：本段也進 NLQ 的「句子是不是在講數理」判斷（mentionsMathPhysics），
    // 化學也常用的詞（理想氣體、絕對溫度、有效數字）不收，免得化學句子被誤判成數理而改走 LLM。
    // 數學：三角、複數、圓錐曲線、計數與機率、矩陣與方程組
    '三角比', '廣義角', '同界角', '象限角', '極坐標', '極座標', '差角公式', '疊合',
    '虛數', '共軛複數', '代數基本定理', '棣美弗定理', '極式', '準線', '焦點', '漸近線',
    '長軸', '短軸', '正焦弦', '文氏圖', '取捨原理', '排容原理', '數學歸納法', '貝氏定理',
    '獨立事件', '二項分布', '幾何分布', '伯努利試驗', '高斯消去法', '增廣矩陣', '轉移矩陣',
    '線性變換', '線性規劃', '可行解', '目標函數', '插值多項式',
    // 物理：新章（測量、質心與角動量、氣體動力論）
    '不確定度', '因次分析', '質心', '角動量守恆', '氣體動力論'
]);

// 〔章節重整 CH-A〕跨科的新名詞：要進詞典（切得出來），但**不進** MATH_PHYSICS_TERMS——
// 化學也教（108 化學的原子構造講電磁波與光譜；氣體單元也可能談分子的方均根速率），
// 收進數理名詞會讓「化學 電磁波的計算」這種明寫化學的句子被判成數理而改走 LLM。
// 同理，這兩個詞也不收成物理章的別名（config/chapterAliases.js「電與磁的統一」「理想氣體與氣體動力論」）。
const SHARED_TERMS = Object.freeze(['電磁波', '方均根速率']);
...
const EXAM_TERMS = [...MATH_PHYSICS_TERMS, ...SHARED_TERMS, ...CHEMISTRY_TERMS];
