...
// 全部是通用學科名詞，非任何考卷內容。
// 〔stage5 WS-B〕原本的 EXAM_TERMS 拆成數理（本段，內容與順序一字未動）與化學兩段，
// 合併後的詞典與拆開前逐字相同；數理這段另外匯出給 NLQ 判斷「句子是不是在講數理」
//（utils/nlqHeuristics.js 的 mentionsMathPhysics）。
const MATH_PHYSICS_TERMS = Object.freeze([
    // 數學：代數與函數
...
// 〔stage5 WS-B〕化學（DEC-019）。同樣只收通用學科名詞；會出現在數學／物理題幹的泛詞
//（反應、平衡、溶液以外的「分壓」「電位」這類）不收。
//
// ⚠ 詞典一改，**已入庫題目的 search_tsv 就與查詢端不一致**（search_tsv 是寫入當下切好存起來的）。
//    本批與化學章節名（經 expandChapterWords）確實會改變部分既有數理題幹的切法，例如
//    「質量數為 238」由「質量／數為」變成「質量數」、「理想氣體」「週期表」「反應速率」由兩個詞變成一個詞。
//    eval fixture 語料剛好沒有這些詞（重切逐字相同），但老師的真實題庫一定有。
//    所以併入後必須跑一次 `npm run search:reindex`（scripts/reindex_search_tsv.js，只重切、不呼叫 LLM），
//    之後再改本檔或章節名也一樣（docs/chemistry.md 第 4.3 節）。
const CHEMISTRY_TERMS = Object.freeze([
    // 化學：物質、原子與計量
    '化學式', '化學反應式', '反應式', '莫耳數', '莫耳濃度', '體積莫耳濃度', '重量莫耳濃度',
    '原子量', '分子量', '亞佛加厥數', '原子序', '質量數', '同位素', '週期表', '限量試劑',
    '產率', '實驗式', '分子式', '示性式', '結構式', '化合物', '混合物', '純物質',
    // 化學：鍵結、結構與能量
    '離子鍵', '共價鍵', '金屬鍵', '氫鍵', '電子組態', '原子軌域', '混成軌域', '游離能',
    '電負度', '極性分子', '非極性分子', '凡得瓦力', '反應熱', '生成熱', '燃燒熱',
    '赫斯定律', '吸熱反應', '放熱反應', '活化能', '催化劑', '反應速率', '速率常數',
    // 化學：平衡、溶液、酸鹼與氧化還原
    '化學平衡', '平衡常數', '勒沙特列原理', '溶度積', '溶解度', '飽和溶液', '溶質', '溶劑',
    '沉澱', '酸鹼中和', '滴定', '當量點', '指示劑', '緩衝溶液', '解離常數', '氧化劑',
    '還原劑', '氧化數', '半反應', '鹽橋', '電解', '電鍍', '理想氣體', '蒸氣壓',
    '沸點上升', '凝固點下降', '滲透壓',
    // 化學：有機
    '有機化合物', '官能基', '同分異構物', '烷類', '烯類', '炔類', '芳香烴', '醇類',
    '酚類', '醚類', '醛類', '酮類', '羧酸', '酯類', '酯化', '胺類', '醯胺', '胺基酸',
    '聚合物', '加成聚合', '縮合聚合'
]);

const EXAM_TERMS = [...MATH_PHYSICS_TERMS, ...CHEMISTRY_TERMS];
...
module.exports = {
    tokenize,
    // 〔stage5 WS-B〕數理名詞（NLQ 的科目判斷用；本身不是分詞介面的一部分）
    MATH_PHYSICS_TERMS
};
