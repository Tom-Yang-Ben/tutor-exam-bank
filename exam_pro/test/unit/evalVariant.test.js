...
describe('eval/golden/variant.json 的硬閘門（第 8.4 條）', () => {
    ...
    test('涵蓋兩科 13 章與五種題型（證明題的 verify 會 skipped，那條路徑必須量得到）', () => {
        const golden = suite.loadVariantGolden({ fixtureById: fixture.byId });
        ...
        // 「廣義角與極坐標」（var-014～016）（eval/CHAPTER_RELABEL-2026-09.md）。
        // 〔CR-8 之二〕10 → 13 章：藍本照 fixture 改標，新增「平面運動」（var-023，藍本 #46）、
        // 「物體的運動（速度與加速度）」（var-026、var-027，藍本 #51、#53）、「靜電學」（var-028，藍本 #55）
        // （同檔第 8 節）。仍是精確值，不是放寬成 ≥。
        assert.equal(chapters.size, 13);
