...
// 每次 CI 都把 92 筆分類標籤、250 個答案比對案例、30 組重複判定重新過一次閘門。
// golden 是純檔案，沒有 DB 的 CHECK 幫忙擋；一個手滑改錯的章節名不會讓任何東西壞掉，
// 只會讓 accuracy 安靜地少 1/92，看起來像模型退步。
...
describe('classify golden（章節分類）', () => {
    test('共 92 筆＝ 62 題 fixture + 30 筆漂移變體', () => {
        // 〔章節重整 CH-B〕90／60 → 91／61：fixture 新增自製干擾題 #61，fixture 段照「標籤沿用 fixture」補上
        // cls-fx-061（eval/CHAPTER_RELABEL-2026-09.md 第 2.1 節）。
        // 〔CR-8 之二〕91／61 → 92／62：fixture 新增自製干擾題 #62，同理補上 cls-fx-062（同檔第 8 節）。
        // 漂移段不變。仍是精確值。
        assert.equal(classify.entries.length, 92);
        assert.equal(classify.entries.filter(e => e.source === 'fixture').length, 62);
...
describe('normalizeStem 轉接層與 scripts/backfill_text_hash.js 的參考實作一致', () => {
    ...
    test('對 fixture 62 題與 dedup golden 的 60 段文字產出逐位元相同的雜湊', () => {
        const texts = [
