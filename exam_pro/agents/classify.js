...
const FEW_SHOT_K = 5;              // 向量最近鄰取幾題（第 3.3 條）
...
async function fewShotByVector(ctx, { subject, chapter, question_text }) {
    ...
    // chapter_src='human' 的標籤是人驗過的，優先當範例；'knn' 沒有任何人驗過，
    // 不得再當範例來源（規劃 §4.3.3：錯一題會自我強化成一串同錯題）。
    const { rows } = await ctx.db.query(
        `SELECT id, chapter, question_text
          ...
          LIMIT $3`,
        [subject, pgvector.toSql(vectors[0]), FEW_SHOT_K]
    );
...
async function run(ctx, input = {}) {
    try {
        ...
        // ── 第二層：few-shot + LLM ──
        const { examples, ids } = await gatherFewShot(ctx, { ...input, subject, question_text: questionText });
