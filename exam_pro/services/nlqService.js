...
//     （平面／空間向量、運動學／受力），界線取自 config/kc 的知識點（名稱與說明）與 config/chapterAliases.js 的別名歸屬；
//     〔dec/x-nlq-improve-fix〕原稿的「力的平衡 → 牛頓運動定律」不在 KC 與別名裡、而且與 golden nlq-036 的 note 用字相同，
//     改為 KC〈牛頓第一定律〉說明裡的「合力為零」。這一條提示（加上「斜面上的受力 → 摩擦力與向心力」）仍直接指向
//     nlq-036 的兩個期望章：重錄後 nlq-036 的結果是「有提示」的表現，不是模型自己判斷的（docs/retrieval.md 第 9.3 節）；
//   - 新增三個【範例】：題材刻意避開 eval/golden/nlq.json 的 50 句（單元測試釘住不得含 golden 原句）。
// 伺服器端另有兩道不依賴模型的檢查（mergeLlm）：句子沒提到的題型／難度／學生不採用、只明講平面或只明講空間時，
// 模型只挑了另一個維度的章就換成同名的那一章（兩章都列、或句子兩種都講時不換）。
...
【容易混淆的章】
- 數學的向量分平面、空間兩組。平面：向量的加減與係數積（加減、係數積、平行、線性組合）、向量內積（夾角、垂直、投影、長度）、面積與行列式。空間：空間概念與座標系、空間向量內積、外積、平面方程式、空間直線方程式。句子寫「平面上」、只有兩個坐標分量、或沒提到空間時用平面那一組；寫了「空間」「三維」或有三個坐標分量才用空間那一組；平面、空間兩種都要時，兩組各列對應的章。
...
- 物理的力學：只問位置、速度、加速度怎麼隨時間變、不談力，是「直線運動」「平面運動」；談到受力、合力（包括合力為零）、張力、正向力或連體，是「牛頓運動定律」；有摩擦力、斜面上的受力或圓周運動的向心力，是「摩擦力與向心力」；有力矩或轉動才是「剛體轉動與平衡」。
 ...
 *   - 章節：句子只明講平面或只明講空間時，挑錯維度的章換成同名的另一章（alignChapterDimension）；
 *     同名的另一章模型已經列了、或句子兩種都講（「平面和空間的都要」）時不換。
