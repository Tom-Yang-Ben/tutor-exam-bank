...
//
// ⚠️ TODO（WS-C 合入後）：下面的 normalizeStem 是「介面第 4 條」規則的自含實作，
//    因為本腳本比 utils/normalizeStem.js 早寫。WS-C 的 A-T5 合入 main 後，
//    這裡要改成 `const { normalizeStem, textHash } = require('../utils/normalizeStem');`
//    並刪掉本檔的 normalizeStem／textHash，且必須確認兩者對題庫全量產出相同的雜湊
//    （驗法：先跑 --dry-run 存一份 --report，換實作後再跑一次比對 JSON）。
