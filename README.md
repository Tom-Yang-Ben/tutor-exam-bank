...
> 我自己當家教用的出題工具：考卷 PDF 丟進去，多 Agent 管線自動拆題入庫；
> 出卷時自動避開學生寫過的題，匯出的 Word 是真的可編輯的直式公式。
> 後來又加上 RAG 檢索（找相似題、用白話搜題）和一個會自己決定調用哪個工具的對話式助教。

repo 裡留著完整的開發歷程：早期原型（`exam/`）、重構後的本體與四個階段的演進（`exam_pro/`）、所有設計文件和決策紀錄（`docs/`）。

> ⚖️ 這是**我自用的工具＋技術作品集**：版權所有、只開放瀏覽與評估，**不授權使用**（見 [`LICENSE`](./LICENSE)）。repo 內不含任何題庫或考卷內容，示範題與 eval 素材都是我自己寫的（見 [`NOTICE`](./NOTICE)）。
...
四個階段是由四條平行 workstream（git worktree）同時施工的，靠「先凍結介面、有爭議就出裁決」的制度整合。所有裁決都留在 [`docs/interfaces*.md`](./docs)，交接紀錄在 [`docs/HANDOFF.md`](./docs/HANDOFF.md)。
...
## 🔍 技術選型 ①：RAG——選了什麼、為什麼、為什麼不用別的

> 更完整的版本（含檔案地圖與常見追問）在 [`docs/rag-and-agents.md`](./docs/rag-and-agents.md)。這一節可以獨立看懂，所有數字都是 `eval/` 實際跑出來的。
...
1. **題庫就幾百到幾千題**。這個量 pgvector 直接全掃就夠快了，連 ANN 索引都還不用開。
2. **關聯條件跟向量檢索得在同一句 SQL 裡**，這是最關鍵的一點。「排掉這個學生做過的題（`NOT EXISTS attempts`）、排掉同家族的變式、鎖難度、跳過已封存」全是關聯查詢；向量庫如果是另一個服務，就得先多撈一堆回來自己過濾，還要維護兩份資料。放在同一個 PG 裡，一句 SQL 解決，交易一致性也順便有了。
3. **用 hybrid 是因為量出來真的比較好**：Recall@5 從純 `LIKE` 的 0.875 拉到 **1.000**。向量抓得到「換個數字的同一題」，全文抓得到專有名詞和符號，剛好互補。
4. **RRF 不用調參**：兩路的分數根本不能直接比（一邊是餘弦、一邊是全文 rank），加權融合得先正規化再調權重；RRF 只看名次，怎麼樣都穩。
5. **就我一個人在維護**，不想多養一個服務。PG 本來就要用，向量功能裝個 extension 就有了。
...
| 方案 | 它的優點 | 它的缺點 | 為什麼不選 |
|---|---|---|---|
| **專用向量庫**（Pinecone／Milvus／Qdrant／Weaviate） | 億級向量、ANN 成熟、託管免維運 | 多一個服務或月費；關聯過濾要 metadata filter 或 over-fetch；與 PG 兩份真相要同步 | 量級差了三個數量級；「排掉做過的題」這種 join 是核心需求，拆成兩個庫是自己找麻煩 |
| **FAISS／記憶體索引** | 最快、零外部服務 | 無持久化、無過濾語意、重啟重建、與資料庫脫節 | 一人專案要「開機就能用」；過濾需求同上 |
| **Elasticsearch／OpenSearch** | 全文檢索最強、也有 kNN | JVM 重、維運重、中文一樣要裝分詞插件 | 為了全文檢索多養一頭大象，划不來 |
...
## 🤖 技術選型 ②：多 Agent 協作——選了什麼、為什麼、為什麼不用別的

### 改造前長什麼樣（repo 裡真實的舊版，不是我編的對照組）

舊版就是**一個巨大的 prompt** 拆整份 PDF：沒有 schema、沒有重試，`JSON.parse` 完直接回——一題壞掉，**整批 400**（真的發生過）。章節白名單在 prompt 裡手抄一份、程式裡又一份，兩邊各自漂移；答案是拆題模型自己抄的，抄錯了沒有任何人抓得到。
...
3. **這裡說的「協作」是互相檢查，不是讓一堆模型開會聊天**：verify 用另一個模型獨立把答案重算一遍再比對（同一個模型抄錯答案時，錯得像模像樣，自己看不出來）；kNN 投票只認**人工確認過**的標籤——自動標籤餵回自動投票，錯一題會自我放大。
4. **省錢是設計進去的，不是事後才想**：閘門照「便宜的先做」排（純文字比對 → embedding → 最後才是 LLM）；檢索找得到就不生成；kNN 夠有把握就整個跳過 LLM；便宜模型拆題、貴模型只用在驗答；再加上 job／每日成本上限和逐 token 記帳。
5. **可測**：CI 以 record/replay cassette 零成本、零網路、**確定性**地跑完整條管線；replay miss 在 main 上是錯誤。

### 缺點（有一說一）

- 前期寫的東西很多：狀態機、閘門、eval 基建全部手寫，起步比套框架慢。
- 比較硬：想加一個節點，DDL 的 CHECK、契約、閘門、測試都要跟著動。
...
> 變式的 gate_pass_rate 只有 0.25，這背後有個值得講的故事：跑題閾值一開始拿「現成題對」校準出 0.92，第一次真實錄製才發現，校準用的正類（只換數字的題對）剛好就是另一道閘門要退回的東西——兩道閘門對「合格」的定義互相矛盾。後來照實測分佈下修到 0.90 並重錄（裁決 S3-R29）。量測的重點從來不是讓數字好看，是**錯了的時候看得見**。
...
### 程式碼：版權所有，不開放使用
這個 repo 是我的個人工具與作品集，公開的目的只有一個：讓人**看**我做了什麼、怎麼做的。
所以程式碼**保留所有權利**——可以在 GitHub 上瀏覽，但不授權執行、複製、修改、散布，
也不允許拿去餵資料集或訓練模型。完整條款見 [`LICENSE`](./LICENSE)。
（2026-08-24 以前的歷史版本曾採 Apache-2.0，該授權僅及於那些歷史版本。）
